...
<body>
	...
	<c:choose>
		<%-- 
			...
			1) null 인 경우 true 반환
			2) 연산 대상이 배열/컬렉션인데 내부에 아무 요소도 없는 경우 true 반환
			3) 나머지는 false 반환
	...
	<h3>5-2) 일반 for 문 + 컬렉션 중 List 사용하기</h3>
	<ul>
		<li>
			nameList : ${nameList}
		</li>
		...
		<li>
		<%--
			 접두사 fn 이 추가된 EL 구문 사용해보기
			
			 fn(function, 기능) : 문자열, 컬렉션 관련 기능을 제공하공 JSTL-EL
	...
	<ul>
		<c:forEach var="name" items="${nameList}" varStatus="vs">
			...
			<c:if test="${vs.last}">	<%-- 반복문의 마지막 번째 순회일 때 true --%>
				<li>------------끝------------</li>
			</c:if>
